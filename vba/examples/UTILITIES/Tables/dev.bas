...
Private Sub Test_FormatConfig_Borders()

    Dim fc As FormatConfig
    Set fc = New FormatConfig
    
    Debug.Print fc.fontName

    Set fc = Nothing

End Sub

Private Sub Test_WrapTextInColumns()
    WrapTextInColumns ActiveSheet.ListObjects(1), "Hello", False
End Sub

Private Sub Test_VerticalAlignColumns()
    VerticalAlignColumns ActiveSheet.ListObjects(1), , xlVAlignCenter
End Sub
Private Sub Test_HorizontalAlignColumns()
    HorizontalAlignColumns ActiveSheet.ListObjects(1), "ID, Comment", xlHAlignLeft
End Sub

Private Sub Test_ResetTableRowHeights()
    ResetTableRowHeights ActiveSheet.ListObjects(1)
End Sub

Private Sub Test_ApplyComfyRowsToTable()
    sta = Timer
    ApplyComfyRowsToTable ActiveSheet.ListObjects(1)
    endT = Timer
    Debug.Print (endT - sta) / 100 * 1000
End Sub

Private Sub Test_ConvertTableToRange()
    ConvertTableToRange ActiveSheet.ListObjects(1)
End Sub

Private Sub Test_AutoNumberColumn()
    AutoNumberColumn ActiveSheet.ListObjects(1), "ID", _
        overwriteExisting:=True, useGUIDs:=False
End Sub

Private Sub Test_TimestampColumn()
    TimestampColumn ActiveSheet.ListObjects(1), "Date", False
End Sub

Private Sub Test_ApplyDefaultValue()
