...
' * Tests
' */
Public Sub TestStringFunctions()
...
' */
Public Function TestToStringFunction() As TestSuite
    With test("To String Function")
        Set TestToStringFunction = .Self
        With .test("Primitave Types")
            .Expect(ToString("test")).ToEqual """test"""
            .Expect(ToString(31)).ToEqual "31"
        End With
        
        With .test("Arrays")
            .Expect(ToString(Array(1, 2, 3))).ToEqual "[1, 2, 3]"
        End With
        
        With .test("Collections")
            Dim col As New Collection
            col.Add "item", "key"
            .Expect(ToString(col)).ToEqual "{""item""}"
        End With
        
        With .test("Dictionary")
            Dim dic As New Scripting.Dictionary
            dic.Add "Name", "Robert"
            dic.Add "Age", 31
            
            .Expect(ToString(dic)).ToEqual "{""Name"": ""Robert"", ""Age"": 31}"
            Debug.Print
        End With
        
        With .test("Objects")
            Dim fso As New FileSystemObject
            .Expect(ToString(fso)).ToEqual "FileSystemObject {}"
            Debug.Print
        End With
    End With
End Function
